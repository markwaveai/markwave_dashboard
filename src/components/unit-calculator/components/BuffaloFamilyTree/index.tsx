...
export default function BuffaloFamilyTree() {
    ...
    const runSimulation = () => {
        setLoading(true);
        {
            ...
            const simulationUnits = 1; // Force 1 unit for Tree View
            for (let u = 0; u < simulationUnits; u++) {
    ...
    const scaledTreeData = React.useMemo(() => {
        ...
        const multiplier = units; // Use the state units explicitly
        // If simulation ran for 1 unit, we multiply by user selection.

        return {
            ...treeData,
            units: units, // Project N units
            summaryStats: {
                ...treeData.summaryStats,
                totalBuffaloes: (treeData.summaryStats?.totalBuffaloes || 0) * multiplier,
            ...
            revenueData: {
                ...
                totalRevenue: (treeData.revenueData?.totalRevenue || 0) * multiplier,
                totalUnits: (treeData.revenueData?.totalUnits || 0) * multiplier,
                averageAnnualRevenue: (treeData.revenueData?.averageAnnualRevenue || 0) * multiplier,
                totalMatureBuffaloYears: (treeData.revenueData?.totalMatureBuffaloYears || 0) * multiplier,
                yearlyData: treeData.revenueData?.yearlyData?.map((d: any) => ({
                    ...d,
                    activeUnits: (d.activeUnits || 0) * multiplier,
                    revenue: (d.revenue || 0) * multiplier,
                    totalBuffaloes: (d.totalBuffaloes || 0) * multiplier,
                    producingBuffaloes: (d.producingBuffaloes || 0) * multiplier,
                    nonProducingBuffaloes: (d.nonProducingBuffaloes || 0) * multiplier,
                    matureBuffaloes: (d.matureBuffaloes || 0) * multiplier,
                })) || []
            }
            // buffaloes and lineages remain unscaled (1 unit representative)
        };
    }, [treeData, units]);

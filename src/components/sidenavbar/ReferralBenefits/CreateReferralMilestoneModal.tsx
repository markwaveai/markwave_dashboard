...
import { X, Gift, Type, ShoppingBag, Send, Star, CheckCircle2, XCircle } from 'lucide-react';
import { referralBenefitService } from '../../../services/api';
...
const CreateReferralMilestoneModal: React.FC<CreateReferralMilestoneModalProps> = ({ isOpen, onClose, onSuccess, milestone }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let result;
            if (milestone) {
                result = await referralBenefitService.updateReferralMilestone(milestone.id, formData);
            } else {
                result = await referralBenefitService.createReferralMilestone(formData);
            }
